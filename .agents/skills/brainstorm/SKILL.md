...
2. **Ask ONE question at a time.** Multiple questions get shallow answers.
3. **Prefer multiple choice when possible.** Choices are easier to answer than open-ended questions, reduce cognitive load, and reveal preferences faster. Only use open-ended when the answer truly cannot be predicted.
...
After all phases are complete, produce the **Improved Prompt** incrementally:

1. **Present in sections of 200-300 words.** After each section, ask: *"Does this section look right so far?"*
2. **Adjust before continuing.** Fix issues in the current section before moving to the next.
3. **Assemble the final prompt** once all sections are validated.
