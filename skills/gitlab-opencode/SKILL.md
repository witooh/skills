...
| "แก้", "fix", "แก้ตาม", "แก้ issue", "implement", "ทำตาม" | **MR Fix** — review first, then ask user to invoke /neo-team-opencode to implement fixes |
| "fix CI", "fix pipeline", "แก้ pipeline", "pipeline fail", "CI fail", "build fail" | **MR CI Fix** — fetch failed job logs, analyze, ask user to invoke /neo-team-opencode to fix |
...
This skill delegates review work to three specialist agents. Their detailed instructions live in the neo-team-opencode skill's reference files. Before spawning review agents, read these files with `read`:

| Specialist | Reference File |
|------------|---------------|
| Code Reviewer | `.opencode/skills/neo-team-opencode/references/code-reviewer.md` |
| Security | `.opencode/skills/neo-team-opencode/references/security.md` |
| QA | `.opencode/skills/neo-team-opencode/references/qa.md` |

> **Fallback paths:** If the above paths don't exist, try `.agents/skills/neo-team-opencode/references/` or `.claude/skills/neo-team-opencode/references/`. OpenCode discovers skills from multiple directories.
...
task(
  ...
  prompt: """
# Role: Code Reviewer
[paste full code-reviewer instructions from neo-team-opencode/references/code-reviewer.md]
...
[paste full security instructions from neo-team-opencode/references/security.md]
...
task(
  ...
  prompt: """
# Role: QA Reviewer
[paste full QA instructions from neo-team-opencode/references/qa.md]
...
When intent is detected as "fix" (user wants to fix issues from a MR), run review first then hand off to /neo-team-opencode for implementation.
...
5. Ask user to invoke /neo-team-opencode with compiled findings
...
### Step 5: Invoke /neo-team-opencode

Use the `skill` tool to invoke neo-team-opencode with the compiled findings:

```
skill({ name: "neo-team-opencode" })
```

Then provide the task context:
...
/neo-team-opencode will classify this as a **Bug Fix** workflow internally and run:
...
After /neo-team-opencode completes, post a summary comment on the MR:
...
---
*Fix โดย GitLab Skill + Neo Team · OpenCode*
```
...
4. Invoke /neo-team-opencode to implement fixes
...
### Step 4: Invoke /neo-team-opencode

Use the `skill` tool to invoke neo-team-opencode:

```
skill({ name: "neo-team-opencode" })
```

Then provide the CI failure context:
...
/neo-team-opencode will classify this as a **Bug Fix** workflow and route through system-analyzer → developer → verification agents.

### Step 5: Push and Retry

After /neo-team-opencode completes:
...
---
*CI Fix โดย GitLab Skill + Neo Team · OpenCode*
```
...
3. Invoke /neo-team-opencode with structured feedback
...
**If there are questions**, use the `question` tool to ask the user before proceeding. The user's answers become part of the context for /neo-team-opencode.

### Step 3: Invoke /neo-team-opencode
...
Then provide the feedback context:
...
---
*Feedback addressed โดย GitLab Skill + Neo Team · OpenCode*
```
...
- **Large single-line files** (minified JS, large JSON): the read tool may show partial content — note this in the review if such files are part of the diff
