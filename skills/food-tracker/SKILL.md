...
```bash
curl -s "$STATE_DB_SUPABASE_URL/rest/v1/skill_state?skill_name=eq.food-tracker&key1=eq.entry&data->>food_name=ilike.*ข้าวมัน*&order=key2.desc&limit=5" \
  -H "apikey: $STATE_DB_SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $STATE_DB_SUPABASE_ANON_KEY"
```
...
```bash
curl -s "$STATE_DB_SUPABASE_URL/rest/v1/skill_state?skill_name=eq.food-tracker&key1=eq.entry&key2=eq.2026-03-14&order=created_at.asc" \
  -H "apikey: $STATE_DB_SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $STATE_DB_SUPABASE_ANON_KEY"
```
...
```bash
curl -s "$STATE_DB_SUPABASE_URL/rest/v1/skill_state?skill_name=eq.food-tracker&key1=eq.entry&key2=like.2026-03*&order=created_at.asc" \
  -H "apikey: $STATE_DB_SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $STATE_DB_SUPABASE_ANON_KEY"
```
...
```bash
curl -s "$STATE_DB_SUPABASE_URL/rest/v1/skill_state?skill_name=eq.food-tracker&key1=eq.entry&key2=eq.2026-03-14&select=id,key2,key3,data,created_at&order=created_at.asc" \
  -H "apikey: $STATE_DB_SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $STATE_DB_SUPABASE_ANON_KEY"
```
