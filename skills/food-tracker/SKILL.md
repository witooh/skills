...
|-----|---------|-------------|
| entry | individual food log | `{food_name, date, meal_time, meal_period, phosphorus_mg, potassium_mg, serving_size, source, notes}` |
...
| date | string | "2026-03-14" | YYYY-MM-DD, for date filtering |
| meal_time | string | "2026-03-14T12:00:00" | ISO datetime, Asia/Bangkok |
| meal_period | string | "กลางวัน" | เช้า / กลางวัน / เย็น / ก่อนนอน |
...
```bash
curl -s "$STATE_DB_SUPABASE_URL/rest/v1/skill_state?skill_name=eq.food-tracker&key=eq.entry&data->>food_name=ilike.*ข้าวมัน*&order=created_at.desc&limit=5" \
  -H "apikey: $STATE_DB_SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $STATE_DB_SUPABASE_ANON_KEY"
```
...
```bash
curl -s "$STATE_DB_SUPABASE_URL/rest/v1/skill_state?skill_name=eq.food-tracker&key=eq.entry&data->>date=eq.2026-03-14&order=created_at.asc" \
  -H "apikey: $STATE_DB_SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $STATE_DB_SUPABASE_ANON_KEY"
```
...
Use `like` to match year-month prefix in the date field:

```bash
curl -s "$STATE_DB_SUPABASE_URL/rest/v1/skill_state?skill_name=eq.food-tracker&key=eq.entry&data->>date=like.2026-03*&order=created_at.asc" \
  -H "apikey: $STATE_DB_SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $STATE_DB_SUPABASE_ANON_KEY"
```
...
```bash
curl -s "$STATE_DB_SUPABASE_URL/rest/v1/skill_state?skill_name=eq.food-tracker&key=eq.entry&data->>date=eq.2026-03-14&select=id,data,created_at&order=created_at.asc" \
  -H "apikey: $STATE_DB_SUPABASE_ANON_KEY" \
  -H "Authorization: Bearer $STATE_DB_SUPABASE_ANON_KEY"
```
...
Insert using the log operation above. Set `date` to YYYY-MM-DD and `meal_time` to full ISO datetime, both in Asia/Bangkok timezone.
...
- **Daily breakdown**: group by `data.date`, sum phosphorus_mg and potassium_mg per day, count entries per day
