...
Every design decision must trace back to an AC-ID. If you find yourself designing something that no AC covers, either:

1. It's out of scope — don't design it
2. The AC is missing — flag it as an Open Question for BA

Do not invent requirements. If an AC says "returns HTTP 400 with error code INVALID", your API contract must specify exactly that — not a different status code or error shape.
