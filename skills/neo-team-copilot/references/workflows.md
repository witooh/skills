...
```
Complex task (BA + Architect + plan confirmation):
1. business-analyst    → structure requirements and acceptance criteria
2. architect           → design endpoint/module contract and data flow
   Context: BA's user stories + acceptance criteria + business rules
3. Present plan        → Orchestrator synthesizes Architect's design into an implementation plan
...
2. Present plan        → Orchestrator synthesizes root cause into a fix plan
   Include: root cause summary, affected files/lines, proposed fix approach, risk assessment
   Present to user for confirmation — bugs can have multiple valid fixes with different trade-offs
...
3. Present plan        → Orchestrator presents refactoring plan to user for confirmation
